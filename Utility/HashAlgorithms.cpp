...
string scaf::SHA256(const void *_data, size_t len, bool upper)
{
	...
	size_t size = len + ((len % 64 >= 56) ? (128 - len % 64) : (64 - len % 64));
	p = new byte[size], pe = p + size;
	for(i = 0; i < len; p[i + 3 - 2 * (i % 4)] = data[i], ++i);
	for(p[i + 3 - 2 * (i % 4)] = 128, ++i; i < size; p[i + 3 - 2 * (i % 4)] = 0, ++i);
	*((uint32_t*)(p + size - 4)) = static_cast<uint32_t>(len << 3);
	*((uint32_t*)(p + size - 8)) = static_cast<uint32_t>(len >> 29);
	for( ; p < pe; p += 64)
	{
		for(i = 0; i < 16; W[i] = ((uint32_t*)p)[i], ++i);
		for(i = 16; i < 64; W[i] = (SHA256_s1(W[i - 2]) + W[i - 7] + SHA256_s0(W[i - 15]) + W[i - 16]), ++i);
		A = H0, B = H1, C = H2, D = H3, E = H4, F = H5, G = H6, H = H7;
		for (i = 0; i < 64; ++i)
		{
			t1 = H + SHA256_S1(E) + SHA256_Ch(E, F, G) + K[i] + W[i];
	...
	if(upper)
		sprintf(buf, "%08X%08X%08X%08X%08X%08X%08X%08X", H0, H1, H2, H3, H4, H5, H6, H7);
	else
		sprintf(buf, "%08x%08x%08x%08x%08x%08x%08x%08x", H0, H1, H2, H3, H4, H5, H6, H7);
	delete[] (p - size);
	return string(buf);
