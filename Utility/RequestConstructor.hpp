...
public:
	...
	static bool determineXhr(std::multimap<string, string> &headers);
	static void buildHeaders(std::multimap<string, string> &headers, mg_str *k, mg_str *v);
	static void parseCookies(std::map<string, string> &cookies, const std::multimap<string, string> &headers);
	static void parseQuery(std::map<string, string> &query, mg_str q);
	static string getHostname(std::multimap<string, string> &headers);
